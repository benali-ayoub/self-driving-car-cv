...
# Main control logic integrating joystick, motor control, and data collection
motor = Motor(2, 4, 3, 17, 22, 27)  # Adjust GPIO pins as needed
joystick = JoystickControl()
data_collector = DataCollector()  # Initialize the DataCollector

def cleanup():
    print("Cleaning up...")
    motor.cleanup()
    joystick.quit()
    data_collector.close()  # Close the data collector
    print("Cleanup complete.")

# Register the cleanup function to be called on normal interpreter termination
...
try:
    while True:
        ...
        if speed != 0 or turn != 0:
            motor.move(speed*0.5, turn, 0.1)
            # Capture frame and record data
            data_collector.capture_frame(turn)
        else:
            motor.stop()
